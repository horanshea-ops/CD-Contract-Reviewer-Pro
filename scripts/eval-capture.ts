...
async function main() {
  ...
  const wanted = only ? key.contracts.filter((c) => c.contract === only) : key.contracts;
  if (wanted.length === 0) {
    throw new Error(`No contract named "${only}" in the key. Known: ${key.contracts.map((c) => c.contract).join(", ")}`);
  }
