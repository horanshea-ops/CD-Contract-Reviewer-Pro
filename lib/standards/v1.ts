 ...
 *
 * **Provisional values (2026-09-23).** CD's template leaves blanks such as
 * "[X]%" and "[date]" in its fallback wording. They are filled with typical
 * industry figures, or with wording that points at the contract's own facts, so
 * the full flow can be tested. These are not CD's numbers. ROADMAP.md's
 * "Provisional values for CD to confirm" lists each one, and CD confirms or
 * replaces them. `tests/standards-blanks.test.ts` keeps new blanks out.
 */
