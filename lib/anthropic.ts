...
- A deviation is a finding however narrow the margin. If CD's position is a food and beverage shortfall billed at no more than thirty-five percent and the contract says forty, that is a finding — record it at the library's severity rather than passing over it as close enough. Not recording a finding is for clauses that MEET CD's position, not for ones that miss it by a little.
- severity comes from that clause's severity_default in the standards library. Depart from it only where this contract's own facts justify it — a figure far outside CD's position, an unusually large block, a term that compounds another — and say why in finding_text. Calling everything high is the same as calling nothing high.
