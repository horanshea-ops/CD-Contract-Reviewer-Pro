 ...
 * - The proposal ends a sentence but the contract's sentence carries on past
 *   the quote. Accepting it would leave the rest of the sentence dangling, and
 *   only the model knows whether that rest should stay, so the change is
 *   refused.
 *
...
export type Fit =
  ...
  | { ok: false; reason: "ends_mid_sentence"; detail: string };
...
export function fitToProposal(part: WalkResult, span: LocatedSpan, language: string): Fit {
  ...
  const quote = wordsIn(text, span.start, span.end);
  let start = span.start;
  let end = span.end;
  let fitted = language;

  if (proposal.length >= ANCHOR_WORDS && !sameRun(quote.slice(0, ANCHOR_WORDS), proposal.slice(0, ANCHOR_WORDS))) {
    const before = wordsIn(text, sentenceStart(text, around.from, span.start), span.start);
  ...
  if (proposal.length >= ANCHOR_WORDS && !sameRun(quote.slice(-ANCHOR_WORDS), proposal.slice(-ANCHOR_WORDS))) {
    const after = wordsIn(text, span.end, sentenceEnd(text, span.end, around.to));
