...
const ADVERSE: Record<string, ClauseTerms> = {
  ...
  cancellation: { damages_basis: "gross_revenue", sliding_scale: true, resale_credit: false, rebook_credit: false, liability_free_months: 0, top_tier_pct: 1.0, net_rate_basis: false },
  force_majeure: { standard: "impossible", covers_government_restrictions: false, covers_epidemic: false, covers_unsafe_travel: false, deposit_refund: false, attendee_cancellation_trigger: false },
  fb_minimum: { menu_price_lock_months: 24, shortfall_rate: 1.0 },
  cutoff_date: { days_prior: 45, post_cutoff_group_rate: false },
  walk_relocation: { comparable_accommodation: false, transportation: false, return_upgrade: false, counts_toward_pickup: false, per_night_credit: false },
  mandatory_fees: { disclosed_before_signature: false, undisclosed_waived: false, resort_fee_usd: 35 },
  rebates: { comp_room_ratio: 75, formula_based: false, forfeited_on_attrition: true, fee_nights_count: false },
  ...
  review_audit_dates: { weekly_pickup_reports: false, post_event_report_days: 90, block_audit_right: false, block_review_rights: false },
  insurance_indemnification: { mutual: false, own_negligence_only: false, group_liability_limit_usd: 5000000, mutual_insurance: false },
  ...
  termination_rights: { for_cause_no_liability: false, separate_from_cancellation_scale: false, pandemic_termination: false },
  ...
  named_storm: { cancellation_window_hours: 24 },
  ...
  rate_parity: { guaranteed: false, retroactive_adjustment: false, commission_preserved: false, lowest_group_rate: false },
  ...
  resale_mitigation_duty: { affirmative_duty: false, proceeds_credited: false, records_available: false, damages_due_after_event: false },
  ...
  av_internet: { quotes_honored: false, in_house_av_not_condition: false, bandwidth_specified: false },
};
...
const COMPLIANT: Record<string, ClauseTerms> = {
  ...
  cancellation: { damages_basis: "room_profit", sliding_scale: true, resale_credit: true, rebook_credit: true, liability_free_months: 12, top_tier_pct: 0.7, net_rate_basis: true },
  force_majeure: { standard: "impracticable", covers_government_restrictions: true, covers_epidemic: true, covers_unsafe_travel: true, deposit_refund: true, attendee_cancellation_trigger: true },
  fb_minimum: { menu_price_lock_months: 12, shortfall_rate: 0.35 },
  cutoff_date: { days_prior: 21, post_cutoff_group_rate: true },
  walk_relocation: { comparable_accommodation: true, transportation: true, return_upgrade: true, counts_toward_pickup: true, per_night_credit: true },
  mandatory_fees: { disclosed_before_signature: true, undisclosed_waived: true, resort_fee_usd: 0 },
  rebates: { comp_room_ratio: 40, formula_based: true, forfeited_on_attrition: false, fee_nights_count: true },
  ...
  review_audit_dates: { weekly_pickup_reports: true, post_event_report_days: 30, block_audit_right: true, block_review_rights: true },
  insurance_indemnification: { mutual: true, own_negligence_only: true, group_liability_limit_usd: 1000000, mutual_insurance: true },
  ...
  termination_rights: { for_cause_no_liability: true, separate_from_cancellation_scale: true, pandemic_termination: true },
  ...
  named_storm: { cancellation_window_hours: 72 },
  ...
  rate_parity: { guaranteed: true, retroactive_adjustment: true, commission_preserved: true, lowest_group_rate: true },
  ...
  resale_mitigation_duty: { affirmative_duty: true, proceeds_credited: true, records_available: true, damages_due_after_event: true },
  ...
  av_internet: { quotes_honored: true, in_house_av_not_condition: true, bandwidth_specified: true },
};

type Patch = Record<string, Partial<ClauseTerms> | "absent">;

/**
 * Terms from CD's 2026 template that the existing drafts never mention.
 *
 * Every contract drafted before the terms existed carries this patch, so its
 * key reads each clause as silent on them, which fails CD's position. eval-03
 * is the exception. It states all of them, so every check stays compliant
 * somewhere in the corpus.
 */
const WITHOUT_2026_TERMS: Patch = {
  force_majeure: { attendee_cancellation_trigger: NOT_STATED },
  termination_rights: { pandemic_termination: NOT_STATED },
  rate_parity: { lowest_group_rate: NOT_STATED },
  walk_relocation: { per_night_credit: NOT_STATED },
  rebates: { fee_nights_count: NOT_STATED },
  review_audit_dates: { block_review_rights: NOT_STATED },
  insurance_indemnification: { mutual_insurance: NOT_STATED },
  resale_mitigation_duty: { damages_due_after_event: NOT_STATED },
  cancellation: { net_rate_basis: NOT_STATED },
  av_internet: { bandwidth_specified: NOT_STATED },
};
 ...
 */
function from(base: Record<string, ClauseTerms>, ...patches: Patch[]): Record<string, SpecClause> {
  ...
  for (const patch of patches) {
    for (const [clause, override] of Object.entries(patch)) {
      ...
      const current = out[clause];
      const merged: ClauseTerms = { ...(current === "absent" ? base[clause] : current) };
...
const ALL_SPECS: EvalContractSpec[] = [
  {
    ...
    terms: from(ADVERSE, WITHOUT_2026_TERMS, { commission: "absent", facilities_services: "absent", future_rate_cap: "absent" }),
  },
  {
    ...
    terms: from(COMPLIANT, WITHOUT_2026_TERMS, {
  ...
  },
  {
    ...
    terms: from(ADVERSE, WITHOUT_2026_TERMS, {
      ...WITHOUT_2026_ADDITIONS,
