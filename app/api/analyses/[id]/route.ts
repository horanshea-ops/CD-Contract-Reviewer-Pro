...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: analysis, error } = await admin
    .from("analyses")
    .select(
      "id, associate_id, client_id, filename, storage_path, source_format, status, error, created_at, completed_at, model_id, library_version, intake_route, had_existing_revisions, existing_revision_authors, existing_revision_count"
    )
    .eq("id", id)
    .maybeSingle();
